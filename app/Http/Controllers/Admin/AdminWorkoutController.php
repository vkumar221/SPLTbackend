...
class AdminWorkoutController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('admin.workouts.workouts',$data);
    }
        ...
        if($request->ajax())
        {
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('image',function($row)
                    {
                        ...
                        {
                            $image = '<div class="d-flex;align-items-center">
										    <img src="'.url('assets/admin/uploads/workout/'.$row->workout_image).'" alt="placeholder" width="37" height="37">
										    <span>'.$row->workout_name.'</span>
										  </div>';
                        }
                        return $image;
                    ...
                    ->addColumn('action', function($row){
                        ...
                        $btn = '<a href="'.url('admin/edit_workout/'.$row->workout_id).'" class="btn btn-icon btn-sm btn-info" title="Edit"><i class="fa fa-edit"></i></a> ';

                        // $btn .= '<a href="'.url('admin/workout_gallery/'.$row->workout_id).'" class="btn btn-icon btn-sm bg-violet text-white" title="Gallery"><i class="fa fa-image"></i></a> ';

                        if($row->workout_status == 1)
                        {
                            $btn .= '<a href="'.url('admin/workout_status/'.$row->workout_id.'/'.$row->workout_status).'" class="btn btn-icon btn-sm btn-warning" title="Click to disable" onclick="confirm_msg(event)"><i class="fa fa-ban"></i></a> ';
                        }
                        else
                        {
                            $btn .= '<a href="'.url('admin/workout_status/'.$row->workout_id.'/'.$row->workout_status).'" class="btn btn-icon btn-sm btn-success" title="Click to enable" onclick="confirm_msg(event)"><i class="fa fa-check"></i></a>';
                        }
    ...
    public function add_workout(Request $request)
    {
        ...
        return view('admin.workouts.add_workout',$data);
    }
    ...
    public function create_workout(Request $request)
    {
        if($request->has('submit'))
        {
            ...
            $ins['workout_added_by']           = Auth::guard('admin')->user()->admin_id;
            $ins['workout_added_on']           = date('Y-m-d H:i:s');
            $ins['workout_updated_by']         = Auth::guard('admin')->user()->admin_id;
    ...
    public function edit_workout(Request $request)
    {
        ...
        $data['set'] = 'workouts';
        return view('admin.workouts.edit_workout',$data);
    }
