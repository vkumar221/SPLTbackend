...
class AdminWorkoutPlanController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('admin.workout_plans.workout_plans',$data);
    }
        ...
        if($request->ajax())
        {
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('action', function($row){
                        ...
                        $btn = '<a href="'.url('admin/edit_workout_plan/'.$row->workout_plan_id).'" class="btn btn-icon btn-sm btn-info" title="Edit"><i class="fa fa-edit"></i></a> ';

                        if($row->workout_plan_status == 1)
                        {
                            $btn .= '<a href="'.url('admin/workout_plan_status/'.$row->workout_plan_id.'/'.$row->workout_plan_status).'" class="btn btn-icon btn-sm btn-warning" title="Click to disable" onclick="confirm_msg(event)"><i class="fa fa-ban"></i></a> ';
                        }
                        else
                        {
                            $btn .= '<a href="'.url('admin/workout_plan_status/'.$row->workout_plan_id.'/'.$row->workout_plan_status).'" class="btn btn-icon btn-sm btn-success" title="Click to enable" onclick="confirm_msg(event)"><i class="fa fa-check"></i></a>';
                        }
    ...
    public function add_workout_plan(Request $request)
    {
        ...
        return view('admin.workout_plans.add_workout_plan',$data);
    }
    ...
    public function create_workout_plan(Request $request)
    {
        if($request->has('submit'))
        {
            ...
            $ins['workout_plan_updated_by']    = Auth::guard('admin')->user()->admin_id;
    ...
    public function edit_workout_plan(Request $request)
    {
        ...
        {
            return redirect('admin/workout_plan');
        }

        $data['set'] = 'workout_plans';
        return view('admin.workout_plans.edit_workout_plan',$data);
    }
    ...
    public function add_to_plan(Request $request)
    {
        ...
        return view('admin.workout_plans.workout_selected',$data);
    ...
    public function filter_workout(Request $request)
    {
        ...
        return view('admin.workout_plans.workout_list',$data);
