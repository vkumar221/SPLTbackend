...
class VendorProductController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('vendor.products.products',$data);
    }
        ...
        if($request->ajax())
        {
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('image',function($row)
                    {
                        if($row->product_image == NULL)
                        {
                            $image = '<div class="d-flex;align-items-center">
										    <img src="'.asset(config("constants.vendor_path")."images/placeholder.png").'" alt="placeholder" width="37" height="37">
                    ...
                    ->addColumn('action', function($row){
                        ...
                        $btn = '<a href="'.url('vendor/edit_product/'.$row->product_id).'" class="btn btn-icon btn-sm btn-info" title="Edit"><i class="fa fa-edit"></i></a> ';

                        // $btn .= '<a href="'.url('vendor/product_gallery/'.$row->product_id).'" class="btn btn-icon btn-sm bg-violet text-white" title="Gallery"><i class="fa fa-image"></i></a> ';

                        if($row->product_status == 1)
                        {
                            $btn .= '<a href="'.url('vendor/product_status/'.$row->product_id.'/'.$row->product_status).'" class="btn btn-icon btn-sm btn-warning" title="Click to disable" onclick="confirm_msg(event)"><i class="fa fa-ban"></i></a> ';
                        }
                        else
                        {
                            $btn .= '<a href="'.url('vendor/product_status/'.$row->product_id.'/'.$row->product_status).'" class="btn btn-icon btn-sm btn-success" title="Click to enable" onclick="confirm_msg(event)"><i class="fa fa-check"></i></a>';
                        }
    ...
    public function create_product(Request $request)
    {
        if($request->has('submit'))
        {
            ...
            $ins['product_vendor']             = Auth::guard('vendor')->user()->vendor_id;
            ...
            $ins['product_added_by']           = Auth::guard('vendor')->user()->vendor_id;
            $ins['product_added_on']           = date('Y-m-d H:i:s');
            $ins['product_updated_by']         = Auth::guard('vendor')->user()->vendor_id;
               ...
               foreach($request->attribute_ids as $key => $product_variant_attribute)
                {
                    ...
                    $insVar['product_variant_added_by']    = Auth::guard('vendor')->user()->vendor_id;
                    $insVar['product_variant_added_on']    = date('Y-m-d H:i:s');
                    $insVar['product_variant_updated_by']  = Auth::guard('vendor')->user()->vendor_id;
    ...
    public function update_product(Request $request)
    {
        ...
        if($request->has('submit'))
        {
            ...
            $upd['product_updated_by']         = Auth::guard('vendor')->user()->vendor_id;
            ...
            foreach($request->product_variant_id as $key => $product_variant_id)
            {
                ...
                $variant->product_variant_updated_by  = Auth::guard('vendor')->user()->vendor_id;
    ...
    public function product_gallery(Request $request)
    {
        ...
        $data['set'] = 'product';
        return view('vendor.products.product_gallery',$data);
    }
    ...
    public function drop_image(Request $request)
    {
        ...
        {
            $file = $request->file->store('assets/vendor/uploads/product');
