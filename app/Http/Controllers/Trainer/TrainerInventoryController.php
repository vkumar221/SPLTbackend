...
class TrainerInventoryController extends Controller
{
    public function index(Request $request)
    {
        $data['set'] = 'inventory';
        $data['products'] = Product::where(['product_vendor'=>Auth::guard('trainer')->user()->trainer_vendor_id,'product_status'=>1])->get();
        return view('trainer.inventory.inventory',$data);
    }
        ...
        if($request->ajax())
        {
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('action', function($row){
                        ...
                        $btn = '<a href="'.url('vendor/view_inventory/'.$row->inventory_id).'" class="btn btn-icon btn-sm btn-info" title="View"><i class="fa fa-eye"></i></a> ';
    ...
    public function add_stock(Request $request)
    {
        $data['set'] = 'inventory';
        $data['products'] = Product::where(['product_vendor'=>Auth::guard('trainer')->user()->trainer_vendor_id,'product_status'=>1])->get();
        return view('trainer.inventory.add_stock',$data);
    }
    ...
    public function select_variant(Request $request)
    {
        if($request->ajax())
        {
            ...
            if($variants->count() > 0)
            {
                ...
                return view('trainer.inventory.variant_table',$data);
            }
