...
class TrainerProductController extends Controller
{
    public function index(Request $request)
    {
        $data['set'] = 'product';
        $data['categories'] = Category::where('category_status',1)->get();
        return view('trainer.products.products',$data);
    }
        ...
        if($request->ajax())
        {
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('action', function($row){
                        ...
                        $btn = '<a href="'.url('trainer/edit_product/'.$row->product_id).'" class="btn btn-icon btn-sm btn-info" title="Edit"><i class="fa fa-edit"></i></a> ';

                        // $btn .= '<a href="'.url('trainer/product_gallery/'.$row->product_id).'" class="btn btn-icon btn-sm bg-violet text-white" title="Gallery"><i class="fa fa-image"></i></a> ';

                        if($row->product_status == 1)
                        {
                            $btn .= '<a href="'.url('trainer/product_status/'.$row->product_id.'/'.$row->product_status).'" class="btn btn-icon btn-sm btn-warning" title="Click to disable" onclick="confirm_msg(event)"><i class="fa fa-ban"></i></a> ';
                        }
                        else
                        {
                            $btn .= '<a href="'.url('trainer/product_status/'.$row->product_id.'/'.$row->product_status).'" class="btn btn-icon btn-sm btn-success" title="Click to enable" onclick="confirm_msg(event)"><i class="fa fa-check"></i></a>';
                        }
    ...
    public function add_product(Request $request)
    {
        ...
        return view('trainer.products.add_product',$data);
    }
    ...
    public function edit_product(Request $request)
    {
        ...
        $data['set'] = 'products';
        return view('trainer.products.edit_product',$data);
    }
    ...
    public function product_gallery(Request $request)
    {
        ...
        $data['set'] = 'product';
        return view('trainer.products.product_gallery',$data);
    }
