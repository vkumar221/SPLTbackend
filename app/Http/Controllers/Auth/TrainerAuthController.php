...
class TrainerAuthController extends Controller
{
    ...
    public function forgotPasswordEmailCheck(Request $request)
    {
        ...
        $findUser = Trainer::where('email', $request->email);
        if($findUser->count() > 0){
            try {
                $data['name'] = $findUser->first()->name;
                $data['link'] = @route('trainer.forgot-password-page',['token' => base64_encode($findUser->first()->id)]);
    ...
    public function updateForgotPassword(Request $request)
    {
        ...
        if($findUser->count() > 0){
            try {
                ...
                return redirect()->route('trainer.forgot-password-page',['token' => base64_encode($findUser->first()->id)])->with('error', "Oops something went wrong");
            }
        }else{
            return redirect()->route('trainer.login-page')->with('error', 'User not found please register');
        }
